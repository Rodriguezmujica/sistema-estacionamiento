/**
 * 🔥 FIREBASE OPTIMIZER - SISTEMA HÍBRIDO
 * Optimización para reducir operaciones Firebase
 * 
 * Con 2,189 servicios/mes necesitamos optimizar para evitar límites
 */
...
class FirebaseOptimizer {
  ...
  async batchSync(operations) {
    ...
    // Verificar límites para todo el lote
    for (const op of operations) {
      if (op.type === 'write' && !this.canPerformOperation('writes')) {
        canProceed = false;
        break;
      }
    }
    ...
    if (!canProceed) {
      console.log(`🚫 Sincronización en lote bloqueada - límite alcanzado`);
      return { success: false, reason: 'limit_reached' };
    }
    ...
    console.log(`📦 Procesando lote de ${batch.length} operaciones`);
...
// Instancia global ya creada arriba
